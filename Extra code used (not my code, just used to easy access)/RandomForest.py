...
def runTestProphet():
    ...
    #futuredays = range(LOOKBACK_DAYS, DF2.index[-1] + 1 + LOOKBACK_DAYS)

    start_date = pd.to_datetime(TEST_END)
    end_date = start_date + timedelta(days=PREDICTION_DAYS)

    futuredays = pd.date_range(start_date+timedelta(days=1),end_date,freq='d')
    futuredays = pd.DataFrame(futuredays)
    futuredays.columns = ['ds']
    futuredays['ds']= pd.to_datetime(futuredays['ds'])

    futuredays.to_csv("dataforecast.csv")
    ...
    df_ProphetPrices = pd.DataFrame(columns=['Index','Forecast'])
    DF = pd.DataFrame(y_true)
    df_ProphetPrices['Index'] = range(DF.index[-1] + 1, DF.index[-1] + 1 + PREDICTION_DAYS)
    df_ProphetPrices = df_ProphetPrices.set_index("Index")
    df_ProphetPrices['Forecast'] = y_pred


    futuredays.to_csv("DF.csv")
    DF.to_csv("y_pred.csv")
    df_ProphetPrices.to_csv("df_ProphetPrices.csv")

    # plot expected vs actual
    plt.plot(y_true, label='Actual')
    plt.plot(y_pred, label='Predicted')
    plt.plot(df_ProphetPrices, color="purple", label=f"Predicted {COMPANY} Prophet Future Price")
    plt.legend()
    plt.show()
...
def runTest():
    ...
    prophetForecast = runTestProphet()
    if ENSEMBLE:
    ...
    df_ProphetPrices = pd.DataFrame(columns=['Index','Forecast'])
    DF = pd.DataFrame(prophetForecast)
    df_ProphetPrices['Index'] = range(DF.index[-1] + 1, DF.index[-1] + 1 + PREDICTION_DAYS)
    df_ProphetPrices = df_ProphetPrices.set_index("Index")
    #df_ProphetPrices['Forecast'] = np.array(futurePrice)
